...
def to_csv_file(filename,headers,rows,delimiter=',',newline='\n'):
    with open(filename,"w") as f:
        f.write(f"{delimiter}".join(headers)+f"{newline}")
        for line in rows:
            f.write(f"{delimiter}".join(line)+f"{newline}")


to_csv_file("check.csv",headers_list,data)

with open("check.csv") as output_f:
